...
## [Exercício] Marcas e Propriedades

O COBIT foi sendo construído e aprimorado ao longo dos anos, se tornando uma marca respeitada. COBIT é uma marca registrada por:

- [x] A) ISACA
  > Como aprendemos com o professor nas aulas do curso, COBIT é uma marca registrada pela ISACA.
- [ ] B) COBIT não é uma marca registrada
- [ ] C) Axelos
- [ ] D) PMI

<br>

## [Exercício] Níveis de Certificação

Conhecer as certificações é importante para os seus estudos do COBIT. São os três níveis de certificação em COBIT oferecidos pela ISACA, exceto:

- [ ] A) Avaliador
- [ ] B) Foundation
- [ ] C) Implementação
- [x] D) Auditor
  > Além da certificação Foundation, estudada neste curso, os demais níveis de certificação COBIT são Implementador e Avaliador.

<br>

## [Exercício] Abrangência do COBIT

O COBIT nasceu como um conjunto de documentos norteadores, mas ao longo do tempo ele foi crescendo e abrangendo mais assuntos.Qual a maior abrangência da versão número 5 do COBIT?

- [ ] A) Gerenciamento
- [x] B) Governança Corporativa de TI
  > De acordo com o que aprendemos em aula, o COBIT 5 abrange a Governança Corporativa de TI, sendo essa uma evolução da versão 4.1 que limitava a abrangência a Gestão de TI.
- [ ] C) Auditoria
- [ ] D) Controle

## [Exercício] Versões do COBIT

Estudamos a evolução do COBIT até chegarmos, em 2012, no COBIT 5. O Framework COBIT 5 é finalmente a versão conclusiva. Essa afirmação é:

- [ ] A) Verdadeira. Essa versão vem sendo construída de 1996 para ser a versão final.
- [ ] B) Falso. Essa versão não é a versão final e conclusiva, pois o framework COBIT foi desenvolvido prevista para chegar a 8 versões até 2020.
- [x] C) Falso. Essa versão não é a versão final e conclusiva. O framework COBIT está em constante desenvolvimento e em melhoria contínua.
  > Aprendemos duranto as aulas do curso que assim com qualquer framework, a melhoria contínua deve ser constante sem que se limite a versões conclusivas.
- [ ] D) Verdadeira. Essa versão é conclusiva pois abrange todas as versões anteriores.